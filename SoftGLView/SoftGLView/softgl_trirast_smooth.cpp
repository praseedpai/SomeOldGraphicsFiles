...
static void RasterizeSpan(unsigned char *Color, float *Depth, const int W, int ScanlineY, 
                          float Ax, float Az, float Ca[3],
                          float Bx, float Bz, float Cb[3])
{
  ...
  float ABx=Bx-Ax, ABz=Bz-Az, ABr=Cb[0]-Ca[0], ABg=Cb[1]-Ca[1], ABb=Cb[2]-Ca[2];
  float t=(MinX-Ax)/ABx;
  float Z=Az+ABz*t;
  float R=Ca[0]+ABr*t;
  float G=Ca[1]+ABg*t;
...
//---------------------------------------------------------------------------
void RasterizeTriangle_SMOOTH(unsigned char *Color, float *Depth, const int W, const int H, 
                              ...
                              float Ca[3], float Cb[3], float Cc[3])  // PER-VERTEX RGB COLORS
{
  ...
  if (aStartY<=aEndY)
  {
    ...
    {
      RasterizeSpan(Color,Depth,W,(int)aStartY,ltX,ltZ,ltC,rtX,rtZ,rtC);
      ltX+=ltDX;
      ltZ+=ltDZ;
  ...
  if (bStartY<=bEndY)
  {
    ...
    {
      RasterizeSpan(Color,Depth,W,(int)bStartY,ltX,ltZ,ltC,rtX,rtZ,rtC);
      ltX+=ltDX;
      ltZ+=ltDZ;
      ltC[0]+=ltDC[0]; ltC[1]+=ltDC[1]; ltC[2]+=ltDC[2];
      rtX+=rtDX;
