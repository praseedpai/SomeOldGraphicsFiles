//============================================================================
// softgl_trirast_smooth_tex_pc.cpp : with perspectively correct (PC) color and 
...
static void RasterizeSpan(unsigned char *Color, float *Depth, const int Width, int ScanlineY, 
                          float Ax, float Az, float Aw, float Ca[3], float Ta[2],
                          float Bx, float Bz, float Bw, float Cb[3], float Tb[2],
                          ...
                          int Wrapping, int Filtering, int Function)
{
  ...
  float ABx=Bx-Ax, ABz=Bz-Az, ABw=Bw-Aw,
  ...
  for (; StartX<=EndX; StartX++)
  {
    if (Z<(*Depth)) 
    {
      if (Filtering==0)
      {
        if (Wrapping==0) Texel = GetTexColor_CLAMP_NEAREST(Texture,TW,TH,S/W,T/W);
                    else Texel = GetTexColor_REPEAT_NEAREST(Texture,TW,TH,S/W,T/W);
      }                    
      else                  
      {
        if (Wrapping==0) GetTexColor_CLAMP_LINEAR(Texture,TW,TH,S/W,T/W,BiLinTexel);
                    else GetTexColor_REPEAT_LINEAR(Texture,TW,TH,S/W,T/W,BiLinTexel);
        Texel=BiLinTexel;                    
      }
      ...
      {                         
        Color[0]=(unsigned char)((R/W) * Texel[0]);
        Color[1]=(unsigned char)((G/W) * Texel[1]);
        Color[2]=(unsigned char)((B/W) * Texel[2]);
      }
...
// Texture colors are normalized RGB values in [0,1]. Actually, the fourth 
// component will store 1/w of the original vertex. The colors and texture-
// coords are also perspectively divided, so for example Ca[0] is the original Ca[0]
// divided by w. The  1/w values are interpolated in screen space and at each pixel, 
// we divide the colors and texture-coords by the interpolated 1/w value to obtain
// the perspectively correct values.
                                     ...
                                     int Wrapping, int Filtering, int Function)
{
  ...
  if (aStartY<=aEndY)
  {
    ...
    {
      RasterizeSpan(Color,Depth,W,(int)aStartY,ltX,ltZ,ltW,ltC,ltT,rtX,rtZ,rtW,rtC,rtT,
  ...
  if (bStartY<=bEndY)
  {
    ...
    {
      RasterizeSpan(Color,Depth,W,(int)bStartY,ltX,ltZ,ltW,ltC,ltT,rtX,rtZ,rtW,rtC,rtT,
                    Texture,TW,TH,Wrapping,Filtering,Function);
      ltX+=ltDX;
      ltZ+=ltDZ;
      ltW+=ltDW;
