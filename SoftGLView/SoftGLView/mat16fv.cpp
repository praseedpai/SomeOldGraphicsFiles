...
float* Mult16fv3fv(float *NewV, float* M, float *V)
{
  float tNewV[3];
  tNewV[0] = M[0]*V[0] + M[4]*V[1] + M[8]*V[2];
  tNewV[1] = M[1]*V[0] + M[5]*V[1] + M[9]*V[2];
  tNewV[2] = M[2]*V[0] + M[6]*V[1] + M[10]*V[2];
  Copy3fv(NewV,tNewV);
  return(NewV);
}
...
float* Mult16fv4fv(float *NewV, float* M, float *V)
{
  float tNewV[4];
  tNewV[0] = M[0]*V[0] + M[4]*V[1] + M[8]*V[2] + M[12]*V[3];
  tNewV[1] = M[1]*V[0] + M[5]*V[1] + M[9]*V[2] + M[13]*V[3];
  tNewV[2] = M[2]*V[0] + M[6]*V[1] + M[10]*V[2] + M[14]*V[3];
  tNewV[3] = M[3]*V[0] + M[7]*V[1] + M[11]*V[2] + M[15]*V[3];
  NewV[0]=tNewV[0];
  NewV[1]=tNewV[1];
  NewV[2]=tNewV[2];
  NewV[3]=tNewV[3];
...
{
  M[0]=(r-l)/2;     M[4]=0;           M[8]=0;             M[12]=(r+l)/2;
  M[1]=0;           M[5]=(t-b)/2;     M[9]=0;             M[13]=(t+b)/2;
  M[2]=0;           M[6]=0;           M[10]=(f-n)/-2;     M[14]=-(n+f)/2;
  M[3]=0;           M[7]=0;           M[11]=0;            M[15]=1;
  return(M);  
}
...
float* Screen2WorldXform16fv(float* M, 
                             float X[3], float Y[3], float Z[3], float O[3], // VIEWING
